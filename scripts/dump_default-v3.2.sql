...
CREATE TABLE public.donjons (
    ...
    CONSTRAINT donjons_lvl_check CHECK (((lvl >= 2) AND (lvl <= 30)))
);
...
COPY public.donjons (id, name, timer, lvl) FROM stdin;
1	The Stonevault	33	15
2	The Dawnbreaker	35	8
3	Ara-Kara, City of Echoes	30	22
4	City of Threads	38	12
5	Mists of Tirna Scithe	30	5
6	The Necrotic Wake	36	18
7	Siege of Boralus	36	25
8	Grim Batol	34	11
\.
...
SELECT pg_catalog.setval('public.donjon_done_id_seq', 3, true);
...
ALTER TABLE ONLY public.donjon_done
    ADD CONSTRAINT donjon_done_pkey PRIMARY KEY (id);
